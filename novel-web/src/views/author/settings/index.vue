...
const fetchUserInfo = async () => {
  try {
    const res = await getAuthorInfo(userStore.userId)
...
const handleAvatarUpload = async (options: UploadRequestOptions) => {
  try {
    const res = await uploadFile(options.file as File, FileUploadType.AUTHOR_AVATAR)
...
const changePassword = async () => {
  ...
  try {
    await updateAuthorPassword(userStore.userId, passwordForm.oldPassword, passwordForm.newPassword)
